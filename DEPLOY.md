...
## 1. Backend + WebSocket relay on Render
...
Sanity-check the API: open `https://cryptolens-api.onrender.com/api/health`; it should return `{"status":"ok"}`.

---

## 2. Frontend to Vercel

1. Vercel > **Add New > Project**, then import this repo.
2. The root [`vercel.json`](vercel.json) builds `frontend/` and outputs
   `frontend/dist`, so there's no need to touch the multi-service prompt.
3. Addd
   | Name | Value |
   |---|---|
